...
function CombinePlaylists() {
  ...
  async function combinePlaylists() {
    ...
    if (
      playlists.selectedPlaylist1 === undefined ||
      playlists.selectedPlaylist2 === undefined
    ) {
    ...
    // else missing songs <= 100 we don't need to make extra API calls
      ...
      const results = await addTracksToPlaylist(
        playlists.selectedPlaylist1,
        missingSongs
      );
      if (results.status) {
        onCombineSuccess();
